...
function App() {
  ...
  useEffect(() => {
    saveProgress();
  }, [uncertaintyLevel, yesCount, noCount, currentPath, messageIndex, achievements]);
  ...
  const checkSpecialEffects = (newTotalClicks: number) => {
    // Trigger hacking screen every 15 clicks
    if (newTotalClicks > 0 && newTotalClicks % 15 === 0) {
      setShowHackingScreen(true);
    }
    // Trigger flicker every 5 clicks (but not when hacking screen is active)
    else if (newTotalClicks > 0 && newTotalClicks % 5 === 0) {
  ...
  const handleYes = () => {
    animateTransition(() => {
      ...
      checkSpecialEffects(newTotalClicks);
  ...
  return (
    ...
    >
        ...
        <div className="text-center">
          ...
          >
            ...
            <div className="flex gap-4 justify-center">
              <button
                onClick={handleYes}
                disabled={isAnimating || showHackingScreen}
          ...
          <div className="mt-8 text-center">
            ...
            {showHackingScreen && (
              <p className="text-xs text-red-500 mt-2 font-bold animate-pulse">
                🚨 NEURAL HACK IN PROGRESS - PLEASE WAIT 🚨
              </p>
            )}
      ...
      <HackingScreen
        isActive={showHackingScreen}
        onComplete={() => setShowHackingScreen(false)}
      />
